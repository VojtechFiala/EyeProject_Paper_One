...
M4_attr <- readRDS("post_m4_attr_Long_11_06.RDS")

A_post_tog <- M4_attr

# M3_attr <- readRDS("post_m3_attr_Long.RDS")
...
# Vietnam         0   0   0   0   0   0   0  91


tiff("Posterior__ATTR_Logit_Long_M4.tif",width=32,height=26,units="cm",res=600,compression = "lzw")
...
plot(NULL,xlim=c(-2,2),ylim=c(1,28.5),yaxt="n",bty="n", ylab="", xlab="", cex.axis=0.9, 
     main="Iris Colour - Women", cex.main=1.5)

rect(-2, 0, 2, 28, col="#10101010", border=NA)

segments(-2, c(seq(from=1, to=9, by=1),seq(from=10.5, to=18.5, by=1),seq(from=20, to=28, by=1)), 2, 
         c(seq(from=1, to=9, by=1),seq(from=10.5, to=18.5, by=1),seq(from=20, to=28, by=1)), col="#40808080")

segments(-2, 9.75, 2, 9.75, col="#908080")
rect(-2, 8.5, 2, 9.5, col="#10101010", border=NA)

segments(-2, 19.25, 2, 19.25, col="#908080")
rect(-2, 18, 2, 19, col="#10101010", border=NA)

rect(-2, 27.5, 2, 28.5, col="#10101010", border=NA)


segments(c(-10,-5,0,5,10),0,c(-10,-5,0,5,10),29,col="#80808080",lty=2)
...
#  text(format(round(Coefficients_check$Up_CI[i],2),nsmall=2), x=10, y=ys[i], cex=cex_t)
  for (i in 1:27) {
    c(polygon(c(dens$x,rev(dens$x)),ys[i]*(c(dens$y,rev(dens$y)*(-1))*0.01)+ycoord[i],border=F, lwd = 1.5, col=colpol))
  }
...
rect(-5, 0, 5, 28, col="#10101010", border=NA)
...
M2_attr <- readRDS("post_m2_attr_Long_11_06.RDS")
...
tiff("Sigmas_ATTR_M2_M4.tif",width=20,height=20,units="cm",res=600,compression = "lzw")
