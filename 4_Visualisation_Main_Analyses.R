...
# We do not these variables now, but we may need them later: 
...
load("Model_1_ATTRACT_both_sexes_standardised_together.Rdata")

load("Model_2_SEXTYP_both_sexes_standardised_together.Rdata")
...
# prepair a table of "which part of the maximum" is the given culture's.
...
# Only need to repeat the "L", "a", and "b" suffixes once for each year, not for each group
Range_Iris <- data.frame(
  Levels = c(paste(levels(as.factor(Eye1$Group)), " iris_L", sep = ""),
             paste(levels(as.factor(Eye1$Group)), " iris_a", sep = ""),
             paste(levels(as.factor(Eye1$Group)), " iris_b", sep = "")),
  
  minimumns = c(sapply(Eye1[c("L_iris", "a_iris", "b_iris")], function(col) tapply(col, Eye1$Group, min))),
  
  maximums = c(sapply(Eye1[c("L_iris", "a_iris", "b_iris")], function(col) tapply(col, Eye1$Group, max))),
  
  range = c(sapply(Eye1[c("L_iris", "a_iris", "b_iris")], function(col) tapply(col, Eye1$Group, calc_range))),
  
  sds = c(sapply(Eye1[c("L_iris", "a_iris", "b_iris")], function(col) tapply(col, Eye1$Group, sd)))
)
# as the fraction of maximum sd...

maxsd <- max(Range_Iris$sds)

Range_Iris$sd_unit <- Range_Iris$sds/maxsd


Range_Sclera <- data.frame(
  Levels = c(paste(levels(as.factor(Eye1$Group)), " sclera_L", sep = ""),
             paste(levels(as.factor(Eye1$Group)), " sclera_a", sep = ""),
             paste(levels(as.factor(Eye1$Group)), " sclera_b", sep = "")),
  
  minimumns = c(sapply(Eye1[c("L_sclera", "a_sclera", "b_sclera")], function(col) tapply(col, Eye1$Group, min))),
  
  maximums = c(sapply(Eye1[c("L_sclera", "a_sclera", "b_sclera")], function(col) tapply(col, Eye1$Group, max))),
  
  range = c(sapply(Eye1[c("L_sclera", "a_sclera", "b_sclera")], function(col) tapply(col, Eye1$Group, calc_range))),
  
  sds = c(sapply(Eye1[c("L_sclera", "a_sclera", "b_sclera")], function(col) tapply(col, Eye1$Group, sd)))
)


maxsd <- max(Range_Sclera$sds)

Range_Sclera$sd_unit <- Range_Sclera$sds/maxsd

ranges_iris_sclera <- rbind.data.frame(Range_Iris, Range_Sclera)

# Iris L*, Iris a*, Iris b* - Women 
# Sclera L*, Sclera a*, Sclera b* - Women 
...
A_post_tog <- extract.samples(m1_attr)

S_post_tog <- extract.samples(m2_Sext)


tiff("Posterior__ATTR_Preds_TOGETHER_STANDARDISED_24_02_25.tif",width=32,height=26,units="cm",res=600,compression = "lzw")
...
tiff("Posterior__SEXT_Preds_TOGETHER_STANDARDISED_24_02_25.tif",width=32,height=26,units="cm",res=600,compression = "lzw")
layout(matrix(1:4,byrow=T,nrow=1), widths=c(2.9,2,2,2),heights=1)
