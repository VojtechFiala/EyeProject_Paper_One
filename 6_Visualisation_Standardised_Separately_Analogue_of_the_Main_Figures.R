...
# Set the WD properly, if the objects not present in the same folder! (Which is my case, since I prefer to give them into a different subfolders...)
setwd("D:/GACR_VF/Measured_Eye_Phenotype_And_Ascribed_Characteristics_In_Different_Countries/Final Figures Draft Data/Model Objects Alternatives/DONE_Standardised_separately")

load("Model_3_ATTRACT_BOTH_SEXES_Standardised_separately.Rdata")

load("Model_4_SEXTYP_BOTH_SEXES_Standardised_separately.Rdata")
...
A_post_tog <- extract.samples(m3_attr)

S_post_tog <- extract.samples(m4_Sext)

# Drawing posterior density plots for Attractiveness...

tiff("Figures_S10_Posterior__ATTR_Preds_SEPARATELY_STANDARDISED.tif",width=32,height=26,units="cm",res=600,compression = "lzw")
...
tiff("Figure_S11_Posterior__SEXT_Preds_SEPARATELY_STANDARDISED.tif",width=32,height=26,units="cm",res=600,compression = "lzw")
layout(matrix(1:4,byrow=T,nrow=1), widths=c(2.9,2,2,2),heights=1)
